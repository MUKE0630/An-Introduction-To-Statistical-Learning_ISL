...
> 逻辑回归不直接对响应变量 $Y$  进行建模，而是**对 $Y$  属于某个特定类别的概率建模**
> 
...
我们寻求 $\beta_0$ 和 $\beta_1$ 的估计值，使得每个个体的概率尽可能与观测的状态一致。逻辑回归模型中使用的**似然函数（********************likelihood function********************）**的数学形式为
...
可以将 [式子（4.4）](Classification%20de1ade1befa04bc79200cd723ad36f2f.md)推广为
...
首先选择单个多项式逻辑回归类作为基线（*baseline*）这里我们选择第 K 类，然后用一下模型替代 [模型（4.7）](Classification%20de1ade1befa04bc79200cd723ad36f2f.md)

$$

\operatorname{Pr}(Y=k \mid X=x)=\frac{e^{\beta_{k 0}+\beta_{k 1} x_{1}+\cdots+\beta_{k p} x_{p}}}{1+\sum_{l=1}^{K-1} e^{\beta_{l 0}+\beta_{l 1} x_{1}+\cdots+\beta_{l p} x_{p}}} \\   k = 1，2，……，K-1\tag{4.10}
$$

$$
\Pr(Y=K|X=x)=\dfrac{1}{1+\sum_{l=1}^{K-1}e^{\beta_{l0}+\beta_{l1}x_1}+\cdots+\beta_{l_p}x_p}\tag{4.11}
$$

$$
\log\left(\dfrac{\Pr(Y=k|X=x)}{\Pr(Y=K|X=x)}\right)=\beta_{k0}+\beta_{k1}x_1+\cdots+\beta_{kp}x_p\tag{4.12}
$$

（4.12）表明特征中任意一对类之间的对数优势是线性的

由于基线模型的不同选择，系数估计值会在两个拟合模型之间出现差异，但拟合值(预测得到的值)、任意一对类之间的对数优势以及其他关键的模型输出将保持不变。所以对多项Logistic回归模型中系数的解释必须谨慎，因为它与基线的选择有关。

我们引入 softmax 编码，即无论编码如何，任意一对类之间的拟合值、对数优势和其他关键模型输出都将保持不变。在 softmax 编码中，我们不是选择一个基线类，而是对称地对待所有的类，并假设对于 $k = 1，……，K$

$$
Pr(Y=k|X=x)=\frac{e^{\beta_{k0}+\beta_{k1}x_1+\cdots+\beta_{kp}x_p}}{\sum_{l=1}^Ke^{\beta_{l0}+\beta_{l1}x1+\cdots+\beta_{lp}x_p}}\tag{4.13}
$$

由此公式推出，第 $k$ 类与第 $k'$ 类之间的对数优势比（*log odds ratio*）相等

$$
\log\left(\dfrac{\Pr(Y=k|X=x)}{\Pr(Y=k'|X=x)}\right)=(\beta_{k0}-\beta_{k'0})+(\beta_{k'1}-\beta_{k'1})x_1+\cdots+(\beta_{kp}-\beta_{k'p})x_p\tag{4.14}
$$
...
- 当两类之间存在实质性分离时，Logistic回归模型的参数估计会非常不稳定。
- 如果预测变量 $X$ 在每个类别中的分布近似正态分布，且样本量较小，那么本节的方法可能比逻辑回归更准确
...
$$
Pr(Y=k|X=x)=\frac{\pi_kf_k(x)}{\sum_{l=1}^K\pi_lf_l(x)}  \tag{4.15}
$$

将使用缩写 $p_k(x)=Pr(Y=k|X=x)$ 这就是观测值 $X=x$ 属于第 $k$ 类的后验概率，即给定该观测的预测值，它是该观测属于第 k 类的概率

一般来说，如果我们有来自总体的随机样本，估计 $k$ 是很容易的，我们简单地计算属于第 $k$ 类的训练观测值的分数。然而，估计密度函数 $f_k ( x )$ 要困难得多。接下来有几种方法来估计 $f_k(x)$ ，然后带入上述方程逼近 Bayes 类

---

### 估计 $f_k(x)$的方法

### p = 1 时的线性判别法（*Linear Discriminant Analysis，*LDA）

> 假设 $p = 1$，即只有一个预测变量。为了估计 $f_k(x)$ ，首先对形式做出一些假设：**假设 $f_k(x)$ 是正态分布或高斯分布，**在一维的设定中，正态密度的形式为
> 

$$
f_k(x)=\dfrac{1}{\sqrt{2\pi}\sigma_k}\exp\left(-\dfrac{1}{2\sigma_k^2}(x-\mu_k)^2\right)  \tag{4.16}
$$

$\mu_k$ 和 $\sigma_k^2$ 分别为第 $k$ 类的均值和方差。进一步假设 $\sigma_1^2=\cdots=\sigma_K^2$ ，即所有的 $K$  类都有一个共享的方差项，用 $\sigma^2$ 来表示。公式 （4.16） 带入 （4.15） 中：

$$
p_k(x)=\dfrac{\pi_k\frac{1}{\sqrt{2\pi\sigma}}\:\exp\left(-\frac{1}{2\sigma^2}(x-\mu_k)^2\right)}{\sum_{l=1}^K\pi_l\frac{1}{\sqrt{2\pi}\sigma}\:\exp\left(-\frac{1}{2\sigma^2}(x-\mu_l)^2\right)} \tag{4.17}
$$

$\pi_k$ 表示观测值属于第 $k$  类的先验概率。对公式（4.17）取对数并重新排列项得公式（4.18），不难看出，这相当于将观测分配给了其中的最大类

$$
\delta_k(x)=x\cdot\dfrac{\mu_k}{\sigma^2}-\dfrac{\mu_k^2}{2\sigma^2}+\log(\pi_k) \tag{4.18}
$$

例如，若 $K = 2$ 且 $\pi_1$ = $\pi_2$，则当 $2x( \mu_1-\mu_2 ) > \mu_1^2 - \mu_2^2$ 时，Bayes类将一个观测分配给类1，否则分配给类2，Bayes决策边界为 $\delta_1 ( x ) = \delta_2 ( x )$ 的点，可以证明这等于

$$
x=\dfrac{\mu_1^2-\mu_2^2}{2(\mu_1-\mu_2)}=\dfrac{\mu_1+\mu_2}{2}   \tag{4.19}
$$

我们知道 $X$ 取自每个类内的高斯分布，并且我们知道所涉及的所有参数，在这种情况下，我们可以计算贝叶斯分类。而在实际生活中，我们无法计算贝叶斯分类

在实际应用中，即使我们很确定我们的假设是 $X$ 取自每个类内的高斯分布，为了应用贝叶斯分类，我们仍然需要估计参数 $\mu_1,\dots,\mu_K$，$\pi_1,\dots,\pi_K$ 和 $\sigma^2$ 。线性判别分析( LDA )方法通过将 $\pi_k$，$\mu_k$ ，$\sigma^2$ 的估计值代入式（4.18）来近似线性判别并分析Bayes类。特别地，使用了以下估计：

$$
\begin{aligned}&\hat{\mu}_k\quad=\quad\dfrac{1}{n_k}\sum_{i:y_i=k}x_i\\&\hat{\sigma}^2\quad=\quad\dfrac{1}{n-K}\sum_{k=1}^K\sum_{i:y_i=k}(x_i-\hat{\mu}_k)^2\end{aligned}  \tag{4.20}
$$
...
LDA分类将公式（4.20）和公式（4.21）中给出的估计代入公式（4.18）得到公式 （4.22），并为一个观测 $X = x$ 分配到最大的类

$$
\hat{\delta}_k(x)=x\cdot\dfrac{\hat{\mu}_k}{\hat{\sigma}^2}-\dfrac{\hat{\mu}_k^2}{2\hat{\sigma}^2}+\log(\hat{\pi}_k)  \tag{4.22}
$$
...
> 现在将LDA分类器扩展到多个预测变量的情况。为了做到这一点，我们假设$X = ( X_1, X_2,\cdots ,Xp)$是从多元高斯(或多元正态)分布中得出的，**各类具有其特定的多元高斯均值向量和共同的协方差矩阵**
> 

多元高斯分布假设每个单独的预测变量服从一维正态分布，每一对预测变量之间具有一定的相关性。

为了说明 $p$ 维随机变量 $X$ 具有多元高斯分布，我们记 $X\sim N(\mu,\boldsymbol{\Sigma})$ 其中，$E ( X ) =\mu$ 为 $X$ (具有 $p$ 个分量的向量)的均值，$Cov ( X ) =\boldsymbol{\Sigma}$ 为 $X$
...
在 $p > 1$ 个预测变量的情况下，LDA分类器假设第 $k$ 类中的观测值取自多元高斯分布 $N(\mu,\boldsymbol{\Sigma})$，其中 $\mu_k$ 是特定类别的均值向量，$\boldsymbol\Sigma$ 是所有 $K$ 类共有的协方差矩阵。将第 $k$ 类的密度函数 $f_k( X = x)$ 代入式 [( 4.15 )](Classification%20de1ade1befa04bc79200cd723ad36f2f.md) 并执行一点代数运算，可以发现Bayes类将一个观测值 $X = x$ 分配给它所在的类:

$$
\delta_k(x)=x^T\boldsymbol{\Sigma}^{-1}\mu_k-\dfrac{1}{2}\mu_k^T\boldsymbol{\Sigma}^{-1}\mu_k+\log\pi_k \tag{4.24}
$$

这是式[( 4.18 )](Classification%20de1ade1befa04bc79200cd723ad36f2f.md)的向量/矩阵版本。注意到( 4.24 )式中 $\delta _k ( x )$ 是 $x$ 的线性函数，即LDA决策规则仅通过其元素的线性组合依赖于 $x$。

LDA正试图逼近所有分类器中总错误率最低的Bayes分类器。也就是说，不管错误来自哪个类别，贝叶斯分类都会产生最小的错误分类观测值总数。

贝叶斯分类器将 $x$ 分配给后验概率 $p_k ( X )$ 最大的类。在二分类情况下，这相当于

$$
\Pr(\operatorname Y=\operatorname{k}|X=x)>0.5
$$

然而，如果我们担心错误地预测过多，那么我们可以考虑降低阈值

$$
\Pr(\operatorname{Y}=\operatorname{k}|X=x)>0.2
$$

如何确定哪个阈值是最好的?这样的决策必须基于领域知识，例如与违约相关的成本的详细信息。

ROC曲线图可以同时显示所有可能阈值的两类误差

![Untitled](Classification%20de1ade1befa04bc79200cd723ad36f2f/Untitled%201.png)

一个分类器的总体性能，由( ROC )曲线下面积( AUC )给出，理想的ROC曲线会趋于左上角，AUC越大，分类器越好。

---

### 二次判别分析(*Quadratic Discriminant Analysis* , QDA)

> 与LDA一样，QDA分类的结果是假设每个类别的观测值来自高斯分布，并将参数的估计值插入贝叶斯定理以进行预测。然而，与LDA不同，**QDA假设每个类都有自己的协方差矩阵**。也就是说，它假设来自第 $k$ 类的观测是$X\sim N(\mu_k,\boldsymbol{\Sigma}_k)$，其中 $\boldsymbol\Sigma k$ 是第 $k$ 类的协方差矩阵。
> 

在此假设下，贝叶斯分类器为最大的类分配一个观测 $X = x$

$$
\begin{array}{r c l}{{\delta_{k}(x)}}&{{=}}&{{-\frac{1}{2}(x-\mu_{k})^{T}\Sigma_{k}^{-1}(x-\mu_{k})-\frac{1}{2}\log|\Sigma_{k}|+\log\pi_{k}}}\\ {{}}&{{=}}&{{-\frac{1}{2}x^{T}\Sigma_{k}^{-1}x+x^{T}\Sigma_{k}^{-1}\mu_{k}-\frac{1}{2}\mu_{k}^{T}\Sigma_{k}^{-1}\mu_{k}-\frac{1}{2}\log|\Sigma_{k}|+\log\pi_{k}}}\end{array}\tag{4.28}
$$
...
> 朴素贝叶斯分类器对 $f_1(x),\dots,f_K(x)$ 采取不同的估计策略，我们没有假设这些函数属于特定的分布族(例如多元正态)，而是做了单一的假设：**在第 $k$ 类中，$p$ 个预测变量是独立的。**
> 

数学意义为：$k=1,\dots,K$

$$
f_k(x)=f_{k1}(x_1)\times f_{k2}(x_2)\times\cdots\times f_{k p}(x_p)\tag{4.29}
$$
...
在多元正态分布的情况下，不同预测变量之间的关联由协方差矩阵的非对角线元素概括。
...
$$
\Pr(Y=k|X=x)=\dfrac{\pi_k\times f_{k1}(x_1)\times f_{k2}(x_2)\times\cdots\times f_{kp}(x_p)}{\sum_{l=1}^K\pi_l\times f_{l1}(x_1)\times f_{l2}(x_2)\times\cdots\times f_p(x_p)}\tag{4.30}
$$
...
- 如果 $X_j$ 是定量的，那么另一种选择是对 $f_{kj}$ 使用非参数估计。一个非常简单的方法是对每个类中第 $j$ 个预测器的观测值做直方图。然后我们可以估计 $f_{kj} ( x_j )$ 为第 $k$ 类中与 $x_j$ 属于同一直方图的训练观测值的分数。或者，我们可以使用核密度估计，它本质上是直方图的平滑版本。
...
我们期望在 $p$ 较大或 $n$ 较小的情况下，使用朴素贝叶斯相对于LDA或QDA有更大的收益，因此减小方差非常重要。
...
> 我们现在对LDA、QDA、朴素贝叶斯和逻辑回归进行分析比较。我们在一个包含 $K$ 个类的集合中考虑这些方法，这样我们就可以将一个观测分配给最大化 $Pr( Y = k | X = x)$ 的类。
> 
